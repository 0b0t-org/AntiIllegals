...
public class RevertHelper {
     ...
     * @param location       the location where books / book shulkers should drop
     *                       (if any)
     * @param checkRecursive whether containers in item form should be checked for
    ...
    public static <E extends Event & Cancellable> void revertItemDisplayEntity(@NotNull final Entity entity,
        ...
        if (entity instanceof final ItemFrame itemFrame)
            ...
            revertArmorStand(armorStand, event);
    }
    ...
    public static <E extends Event & Cancellable> void revertArmorStand(@NotNull final ArmorStand armorStand,
        ...
        // Check and revert all armor stand equipment
        for (final EquipmentSlot slot : EquipmentSlot.values()) {
            final ItemStack item = armorStand.getItem(slot);
            if (RevertHelper.revert(item, location, true, ItemState::isIllegal)) {
                deletedIllegal = true;
                armorStand.setItem(slot, new ItemStack(Material.AIR));
            }
        }

        // If any illegal items were found and reverted, cancel the event
        if (deletedIllegal) {
            event.setCancelled(true);
            log(event.getEventName(), "Deleted Illegal from armor stand.");
        }
    }

    /**
     * use this method to check and remove illegal items from inventories (overload
     * of checkInventory with isInsideShulker set to false)
     *
     * @param inventory      the inventory that should be checked
     * @param location       location of the inventory holder for possible item
     *                       drops
    ...
    public static ReversionResult checkInventory(@NotNull final Inventory inventory, @Nullable final Location location,
        ...
        return new ReversionResult(bookItemStacks.size(), wasReverted);
    }
    ...
    private static boolean revertIllegalNBTData(@NotNull final ItemStack itemStack) {
        // bitwise here, since all operations should execute
        return revertColoredName(itemStack)
                ...
                | revertNBTContainers(itemStack)
                | revertBlockStateTags(itemStack)
     ...
     * note: written book names are not affected by this; only the display name is
     * checked
     *
     * @param itemStack the item to revert
    ...
    /**
     * reverts the illegal durability of an item. the durability is clamped into the
     * range from 0 to the max durability
     *
     * @param itemStack the item to revert
    ...
    private static boolean deleteIllegalItem(@NotNull final ItemStack itemStack) {
        ...
        if (!MaterialHelper.isIllegalMaterial(itemStack))
            return false;
     ...
     */
    private static boolean revertNBTContainers(@NotNull final ItemStack itemStack) {
        ...
        NBT.modify(itemStack, nbtItem -> {
            if (MaterialHelper.isBeeContainer(itemStack)) {
                if (sanitizeBeeContainer(nbtItem))
                    modified.set(true);
                return;
            }

            if (!nbtItem.hasTag("BlockEntityTag") && !nbtItem.hasTag("EntityTag"))
                return;

            nbtItem.removeKey("BlockEntityTag");
            nbtItem.removeKey("EntityTag");
            modified.set(true);
        });
    ...
    private static boolean revertBlockStateTags(@NotNull final ItemStack itemStack) {
        if (!AntiIllegals.config().getBoolean("blockStates.enabled"))
            return false;

        final Set<String> blockStateKeys = AntiIllegals.config()
                .getStringList("blockStates.keys")
                .stream()
                .map(String::trim)
        ...
        NBT.modify(itemStack, nbtItem -> {
            final ReadWriteNBT blockStateTag = nbtItem.getCompound("BlockStateTag");
            if (blockStateTag == null)
                return;

            boolean removedAny = false;
            for (final String key : blockStateKeys) {
                if (!blockStateTag.hasTag(key))
                    continue;

                blockStateTag.removeKey(key);
                removedAny = true;
            }
    ...
    private static boolean sanitizeBeeContainer(@NotNull final ReadWriteItemNBT nbtItem) {
        final ReadWriteNBT blockEntityTag = nbtItem.getCompound("BlockEntityTag");
        if (blockEntityTag == null)
            return false;

        boolean modified = removeSensitiveBeeData(blockEntityTag);

        final ReadWriteNBTCompoundList bees = blockEntityTag.getCompoundList("Bees");
        if (bees != null) {
    ...
    /**
     * Removes sensitive bee data from NBT compounds
     *
     * @param compound the NBT compound to sanitize
     * @return whether any sensitive data was removed
     */
    ...
    /**
     * removes conflicting enchantments from an item and only keeps a single one
     * randomly, to resolve the conflict
     *
     * @param itemStack the item to revert
    ...
    private static boolean removeAttributes(@NotNull final ItemStack itemStack) {
        ...
        NBT.modify(itemStack, nbtItem -> {
            if (!nbtItem.hasTag("AttributeModifiers"))
                return;
     ...
     * @return whether attribute modifiers were removed
     */
    private static boolean removeHideFlags(@NotNull final ItemStack itemStack) {
        if (!AntiIllegals.config().getBoolean("hideFlags"))
            return false;

        final AtomicBoolean removed = new AtomicBoolean(false);

        NBT.modify(itemStack, nbtItem -> {
            if (!nbtItem.hasTag("HideFlags"))
                return;

            nbtItem.removeKey("HideFlags");
            removed.set(true);
        });
    ...
    /**
     * Removes suspicious stew effects that exceed normal limits of 240 ticks
     * (highest effect duration)
     *
     * @param itemStack the suspicious stew item to check
     * @return whether any effects were removed or modified
     */
    private static boolean removeSuspiciousStewEffects(@NotNull final ItemStack itemStack) {
        if (!AntiIllegals.config().getBoolean("suspiciousStewEffects"))
            return false;

        if (itemStack.getType() != Material.SUSPICIOUS_STEW)
            return false;
        ...
        // More than 1 effect or any effect with high duration
        for (PotionEffect effect : effects) {
            if (effects.size() > 1 || effect.getDuration() > 240) {
                stewMeta.clearCustomEffects();
                stewMeta.addCustomEffect(SuspiciousEffectEntry.create(PotionEffectType.SATURATION, 7), true);
    ...
    private static boolean removeCustomPotionEffects(@NotNull final ItemStack itemStack) {
        ...
        final List<ItemStack> sanitizedProjectiles = new ArrayList<>();
        boolean modified = false;

        for (final ItemStack projectile : crossbowMeta.getChargedProjectiles()) {
            if (projectile == null || projectile.getType() == Material.AIR)
                continue;

            final ItemStack workingProjectile = projectile.clone();
            final ItemState state = checkItemStack(workingProjectile, null, true);

            if (state.wasReverted())
                modified = true;

            if (state != ItemState.ILLEGAL && workingProjectile.getType() != Material.AIR
                    && workingProjectile.getAmount() > 0)
                sanitizedProjectiles.add(workingProjectile);
        }

        if (!modified)
            return false;

        crossbowMeta.setChargedProjectiles(sanitizedProjectiles);
     ...
     * if the item is enchantable with the enchantment, the level is clamped down to
     * the maximum.
     * if the item is not enchantable and collectibles are enabled, the enchantment
     * level will
     * be clamped down to 1 if greater than 1, or to 0 if less than 0.
     * if the item is not enchantable and collectibles are disabled, the enchantment
     * is removed.
     *
     * @param itemStack the item to revert
    ...
    private static boolean removeIllegalEnchantmentLevels(@NotNull final ItemStack itemStack) {
        if (!AntiIllegals.config().getBoolean("maxEnchantments"))
            return false;
        ...
        for (final Enchantment enchantment : itemStack.getEnchantments().keySet()) {
            ...
            if (allowCollectibles && !MaterialHelper.isArmor(itemStack) && !MaterialHelper.isWeapon(itemStack)) {
                // item is not enchant-able by the enchantment, is not a weapon or armor and
                // lore items are enabled
                if (level < 0 || level > 1) {
                    wasFixed = true;
                    itemStack.removeEnchantment(enchantment);
